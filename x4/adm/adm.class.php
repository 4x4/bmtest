...
class AdminPanel
{
    ...
    public  function __construct()
    {
        ...
        //$this->_EVM->on('AdminPanel:afterCacheClear','warmUpTreeCache',$this);
    }
    ...
    public function listen()
    {
        ...
        {        
          ...
          if (XOAD_Server::runServer())
            {
                    $all=ob_get_contents();ob_end_clean();
    ...
    {            
            ...
            $foldersToClear=array('imagecache','treewarm');
    ...
    public function warmUpTreeCache($params)
    {
        static $treesHolder=array(); 
        if($treeWarmList=XFILES::filesList(xConfig::get('PATH','CACHE').'treewarm','files', null,0, true)){
         
         foreach($treeWarmList as $element)
         {
            if ($data = XCacheFileDriver::readFile('treewarm',$element))
             {
                     $data=unserialize($data);
                    if(!$tree=$treesHolder[$data['treeName']])
                     {
                        $tree=$treesHolder[$data['treeName']]=new XTreeEngine($data['treeName'], XRegistry::get('XPDO'));    
                        $tree->cacheState(true);
                     }
             
                unset($data['treeName']);
                $tree->executeQueryArray($data);
             }
    ...
    {       
        if(!$lang&&$moduleName){$lang = Common::getModuleLang($moduleName, $_SESSION['lang']);}
    ...
    private function applyModuleAccess($modules)
    {
        ...
        while(list($module,$instance)=each($modules))
        {
