...
dhtmlXGridObject.prototype.expandKids=function(curRow,sEv){
   ...
   if(r.childs.length){
    ...
    //x4
    this.callEvent("onItemOpened",[r.id,1]);
   ...
   eXcell_tree.prototype.setValueA = function(content){
	...
	eXcell_tree.prototype.setValue = function(valAr){
		...
		if ((this.grid._tgc.imgURL==null)||(this.grid._tgc.imgURL!=this.grid.imgURL)){
			...
			_tgc.start="<div class='treegrid_cell' style='overflow:hidden; white-space : nowrap; height:"+(_isIE?22:23)+"px;'>";
...
dhtmlXGridObject.prototype.loadByPath=function(path)
{
    
        for(i=0;i<path.length;i++)
        {
            this.openItem('0'+path[i]);
        }
        
 
        this.selectRowById('0'+path[i-1]);


};
dhtmlXGridObject.prototype.getParentPath=function(id,dataItem)
{
    ...
    while(id)
    {
        var pid=this.getParentId(id);
                ...
                {
                    x=this.getRowById(id);
                    p.push(x._attrs.data[dataItem]);
