...
if(!val)return;
	...
	if (val.indexOf("^") != -1){
		...
		this.cell.titFl="1";
         ...
         sub=this.grid.iconURL;
       if(val.indexOf('/')!=-1){sub='';}

    
	this.setCValue("<img src='"+sub+(val||"")._dhx_trim()+"' border='0'>", val);
...
eXcell_imgsmall.prototype=new eXcell;
eXcell_imgsmall.prototype.getTitle=function(){
    return this.cell._brval
};
eXcell_imgsmall.prototype.setValue=function(val){
    var title = val;
if(!val)return;
    ...
    this.setCValue("<img height='70' width='70' src='"+sub+(val||"")._dhx_trim()+"' border='0'>", val);
...
function eXcell_link(a) {
    ...
    this.setValue = function (a) {
        if (typeof a != "number" && (!a || a.toString() == "")) return 0;
        var b = a.split("^");
        b.length == 1 ? b[1] = "" : b.length > 1 && (b[1] = "href='" + b[1] + "'", b[1] += b.length == 3 ? " target='" + b[2] + "'" : " target='_blank'");
        this.cell.innerHTML="<a " + b[1] + " onclick='(_isIE?event:arguments[0]).cancelBubble = true;'>" + b[0] + "</a>";
    }
}
