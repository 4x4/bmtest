...
function dhtmlxArray(ar){
	return dhtmlXHeir((ar||[]), dhtmlxArray._master);
}
dhtmlxArray._master={
...
function dhtmlXGridObject(id){
	...
	this.initCellWidth=[];
	this.fldSort=[];
	this._srdh=(_isIE && (document.compatMode != "BackCompat") ? 24 : 23);
...
dhtmlXGridObject.prototype.groupBy = function (ind, mask) {
    ...
    ltd.innerHTML = "&nbsp;";
    if (mask[i] == "#title")this._gmask._title = rindex; else ltd.align = this.cellAlign[i] || "left";
...
dhtmlXGridObject.prototype._updateGroupView = function (z) {
    ...
    if (this.customGroupFormat)html += this.customGroupFormat(z.text, z.count); else html += z.text + " ( " + z.count + " ) ";
