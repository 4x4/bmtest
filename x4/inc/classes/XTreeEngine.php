...
class xteTree
{
    ...
    public function recursiveStep($startNode, $context, $function, $extdata = null)
    {
        if (isset($this->tree[$startNode])) {
            reset($this->tree[$startNode]);
            while (list(, $node) = each($this->tree[$startNode])) {
...
class XTreeEngine implements \repoProvider
{
    ...
    public $cacheQueryLongerThan = 0.9;
    public $treeName;
    ...
    public function cacheState($enable)
    {
        $this->enableCache = $enable;
    }

    public function revertToMainTables($treeName)
    {

        $oldStructName = strtolower("_tree_" . $treeName . "_struct");
        $oldParamName = strtolower("_tree_" . $treeName . "_param");

        $this->dropTmpTables($oldStructName, $oldParamName);

        $queryStruct = 'CREATE TABLE ' . $oldStructName . ' LIKE ' . $this->treeStructName;
        $this->PDO->query($queryStruct);

        $queryStruct = 'INSERT ' . $oldStructName . ' SELECT * FROM ' . $this->treeStructName;
        $this->PDO->query($queryStruct);

        $queryParam = 'CREATE TABLE ' . $oldParamName . ' LIKE  ' . $this->treeParamName;
        $this->PDO->query($queryParam);

        $queryParam = 'INSERT ' . $oldParamName . ' SELECT * FROM   ' . $this->treeParamName;
        $this->PDO->query($queryParam);

        $this->dropTmpTables($this->treeStructName, $this->treeParamName);

    }

    public function createTempTree($tmpPostFix)
    {
        $newNameStruct = $this->treeStructName . '__' . $tmpPostFix;
        $newNameParam = $this->treeParamName . '__' . $tmpPostFix;

        if ($result = $this->PDO->query("SHOW TABLES LIKE '" . $newNameStruct . "'")) {
            $row = $result->fetch();
            if (!empty($row[0])) return;
        }

        $this->createTmpTables($newNameStruct, $newNameParam);
    }

    public function dropTmpTables($newNameStruct, $newNameParam)
    {

        $queryStruct = 'DROP TABLE ' . $newNameStruct;
        $this->PDO->query($queryStruct);
        $queryParam = 'DROP TABLE ' . $newNameParam;
        $this->PDO->query($queryParam);


    }

    private function createTmpTables($newNameStruct, $newNameParam, $dropBefore = false)
    {
        if ($dropBefore) {
            $this->dropTmpTables($newNameStruct, $newNameParam);
        }

        $queryStruct = 'CREATE TABLE ' . $newNameStruct . ' LIKE ' . $this->treeStructName;
        $this->PDO->query($queryStruct);

        $queryStruct = 'INSERT  ' . $newNameStruct . ' SELECT * FROM ' . $this->treeStructName;
        $this->PDO->query($queryStruct);

        $queryParam = 'CREATE TABLE ' . $newNameParam . ' LIKE ' . $this->treeParamName;
        $this->PDO->query($queryParam);

        $queryParam = 'INSERT ' . $newNameParam . ' SELECT * FROM ' . $this->treeParamName;
        $this->PDO->query($queryParam);
    }

    private function refreshTables($newNameStruct, $newNameParam)
    {

        $qLastChanged = "select max(value) as v from `_tree_'" . $this->treeName . "_struct` where parameter='__nodeChanged'";

        if ($result = $this->PDO->query($qLastChanged)) {
            $row = $result->fetch();
            $originalChanged = $row['v'];
        }

        $qLastChangedTmp = "select max(value) as v from `" . $newNameParam . "` where parameter='__nodeChanged'";

        if ($result = $this->PDO->query($qLastChangedTmp)) {
            $row = $result->fetch();
            $tempChanged = $row['v'];
        }

        if ($originalChanged > $tempChanged) {
            $this->createTmpTables($newNameStruct, $newNameParam, true);
        }


    }

    public function switchToTempTree($tmpPostFix)
    {

        $this->treeStructName = strtolower("_tree_" . $this->treeName . "_struct__" . $tmpPostFix);
        $this->treeParamName = strtolower("_tree_" . $this->treeName . "_param__" . $tmpPostFix);
        $this->cacheDir .= '/' . $this->treeName . '__' . $tmpPostFix;
    ...
    public function setCacheParams($dir, $timeOut = 3600)
    {
        $this->cacheDir = $dir . '/' . $this->treeName;
    ...
    public function run()
    {
        ...
        } elseif (isset($nStructResults) && $nStructResults && !isset($whereQs['whereParamsString'])) {
        ...
        if (isset($nodesIntersect)) {
            ...
            if ($this->enableCache && !isset($this->query['delete'])) {
                ...
                if ($this->cacheQueryLongerThan < $endTime) {
                    ...
                    if ($this->enableCache)
                        XCache::serializedWrite($markQuery, 'treewarm', md5(print_r($markQuery, true)), $this->cacheTimeout);
                }
                if (!($result instanceOf xteTree)) {
            ...
            foreach ($this->query['where'] as $pairs) {
                ...
                if (strpos($pairs[0], '@') === 0) {
                    ...
                    if (is_array($pairs[2]) and ($pairs[1] == '=')) {
                        ...
                        $imp = implode('|', $pairs[2]);
                        $imp = str_replace('%', '', $imp);
                        $whereParams[] = "(parameter = '{$pairs[0]}'  AND value REGEXP '{$imp}')";
    ...
    private function formatProcess($structRecords, $paramsRecords = null, $nodeIntersect)
    {
        ...
        $this->query['pathCache'] = array();
        reset($nodeIntersect);
        while (list($key, $record) = each($nodeIntersect)) {
            $brecord['id'] = $record;
    ...
    public function dataPumpGo()
    {
        ...
        if (isset($this->pumpDataNodeToCheck)) {
            ...
            if (isset($ancestors)) {
                ...
                foreach ($ancestors as $ancestor) {
                    if (isset($this->pumpDataAll[$ancestor['id']])) {
                        reset($this->pumpDataAll[$ancestor['id']]);
                        while (list($k, $v) = each($this->pumpDataAll[$ancestor['id']])) {
