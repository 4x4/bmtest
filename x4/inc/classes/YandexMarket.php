...
class YandexMarket
{
    ...
    public function addCat($sName, $id, $parent)
    {
        $this->aCat[] = "<category parentId=\"{$parent}\" id=\"$id\">" . self::filterElem($sName) . "</category>";
    }
...
class OfferYmt
{
    ...
    protected
        $sDtdElem = 'url?, buyurl?, price, wprice?, currencyId, xCategory?, categoryId+,
                ...
                manufacturer_warranty?,country?,country_of_origin?, downloadable?, adult?,
                barcode*, param*',
        $aOffer = array(),
        $aElems = array(),
        $sOfferElem = '';
