    ...
    extends xModulePrototype
{
    ...
    public function dispatchFrontAction($action, $actionParams)
    {
        ...
        $actionParams['cache'] = $cache = $this->readActionCache($actionParams);
    ...
    private function readActionCache($actionData)
    {
        if (xConfig::get('GLOBAL', 'actionModuleCache') && $actionData['base']['_Cache']) {
            ...
            if ($cacheTemp = XRegistry::get('EVM')->fire($this->_moduleName . '.onModuleCacheRead', array('instance' => $instance, 'actionData' => $actionData, 'cache' => $cache))) {
                $cache = $cacheTemp;
            }
    ...
    public function pushtmsStack($key)
    {
        if (!isset($this->tmsStack[$key])) {
            ...
            if ('HEADLESS' == XRegistry::get('TPA')->getRenderMode()) {
                $this->_TMS = $this->tmsStack[$key] = new MultiSectionHL();
