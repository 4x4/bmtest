            ...
            foreach ($data as $namespace => $functions) {
                ...
                if (!empty($functions)) {
                    foreach ($functions as $fName => $funcParams) {
                        if ($result = XNameSpaceHolder::call($transmit.$namespace, $fName, $funcParams)) {
                    ...
                    if (($instance = XNameSpaceHolder::getInstanceSource($transmit.$namespace,
                            $fName))
                        && (($instance->result) or ($instance->lct))
                    ) {

                        if(empty($this->lct)){$this->lct=array();}
                        if(empty($instance->lct)){$instance->lct=array();}
