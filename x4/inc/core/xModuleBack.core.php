    ...
    extends xModulePrototype
{
    ...
    public function initiateBackActionsCallNS()
    {
    ...
    public function getModuleActions($params)
    {
        if ($actions = $this->_commonObj->getActions()) {
            ...
            while (list($name, $action) = each($actions)) {
    ...
    public function getTemplatesList($actions = '', $useRealPath = false, $recursive = false,
                                     $getInnerModuleTemplates = false, $clearAlias = false, $moduleName = '',$domain='altabeltest.link')
    {
        $isStaticCall = !(isset($this));
        ...
        if ($getInnerModuleTemplates) {
            ...
            $prefixPath = xConfig::get('PATH', 'TEMPLATES').$domain.'/_modules/';
        }

        if (!$isStaticCall) {
            $pathTpl = $prefixPath . $this->_moduleName . $postfix;
        } else {
            $pathTpl = $prefixPath . $moduleName . $postfix;
        }

        if ($allTemplates = XFILES::filesList($pathTpl, $type, $ext, $recursive)) {
