...
class Common
{
    ...
    public static function getModuleTemplateListAsSelector($module, $selected, $extensions = array('.html'), $getAliases = true, $templateBranch='altabeltest.link')
    {

        if ($files = Common::getModuleTemplateList($module, $extensions))
        {
            if ($getAliases)
            {
            ...
            return XHTML::arrayToXoadSelectOptions(XARRAY::combine($aliases, $files), $selected, true);
        }
    }
    
    public static function getModuleTemplateList($module, $extensions = array('.html'),$templateBranch='altabeltest.link')
    {
        if (is_array($files = XFILES::filesList(xConfig::get('PATH', 'TEMPLATES').$templateBranch.'/_modules/'. $module, 'files', $extensions, 0, true)))
        {
            return $files;
        }
    }
    ...
    public static  function getLegacyXLSParser($file)
    {
        require_once(xConfig::get('PATH','EXT').'xls-reader/php-excel-reader/excel_reader2.php');                
        return  new Spreadsheet_Excel_Reader($file);
    }
    
    public static  function getXLSParser($file)
    {
        require_once(xConfig::get('PATH','EXT').'xls-reader/SpreadsheetReader.php');            
            ...
            if ($pagesRealCount)
                while ($i < $pagesRealCount)
                {
                    ...
                    {        
                        ...
                        if($requestParams) {$requestParamsStr.='?'.$requestParams;    
                     ...
                     if($requestParams) {$paginameStr.='&'.$paginame; }else{$paginameStr=$paginame;}
